...
def create_app(
    ...
    async def post_chat_completions(request: Request):
        ...
        logger.info(
            "route source=openai_chat selected=%s reason=%s fallback=%s stream=%s",
            cfg.models[alias].model_id,
            decision.reason,
            used_fallback,
    ...
    async def post_completions(request: Request):
        ...
        headers = _route_headers(cfg, decision, alias, used_fallback)
        logger.info(
            "route source=openai_completions selected=%s reason=%s fallback=%s stream=%s",
            cfg.models[alias].model_id,
            decision.reason,
            used_fallback,
            bool(payload.get("stream")),
    ...
    async def post_anthropic_messages(request: Request):
        ...
        headers = _route_headers(cfg, decision, alias, used_fallback)
        logger.info(
            "route source=anthropic_messages selected=%s reason=%s fallback=%s stream=%s",
            cfg.models[alias].model_id,
            decision.reason,
            used_fallback,
            is_stream,
